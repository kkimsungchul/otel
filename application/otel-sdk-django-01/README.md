...
2. Django의 URLConf는 요청된 URL을 분석하여 해당하는 View 함수나 클래스를 결정하면서 요청을 추적하는 첫 번째 스팬 생성
3. 결정된 View에서 비즈니스 로직 처리, Model을 통해 DB와 상호작용함
4. WSGI 미들웨어를 통해 모든 요청과 응답의 로깅 및 메트릭을 수집하고 이 데이터를 외부 시스템으로 전송
5. 사용자에게 Response 전달
